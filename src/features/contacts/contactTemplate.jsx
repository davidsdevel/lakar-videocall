...
import { useUser } from "@/features/layout/provider";
	...
	return (
		<div className="flex w-full py-2 items-center">
			<div className="relative">
				...
				<div
					className={`w-4 h-4 rounded-full absolute bottom-0 right-0 border-2 border-slate-800 ${online ? "bg-green-500" : "bg-red-500"}`}
				/>
			</div>
			<div className="flex flex-col text-white pl-2 flex-grow">
				<span className="font-bold text-lg">{username}</span>
			</div>
			<button className="mr-2" onClick={() => router.push(`/${_id}`)}>
